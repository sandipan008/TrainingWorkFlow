...
//
//	public void findModule(Task task, Category category, Boolean projectAssignation, String moduleName, String last) {
...
//						log.debug("M Value {} Task Value {}" , m.get(i).getName() , task.getTask());
...
//			}
//			log.debug("category: {} userId: {}", category.getCategory(),category.getUserId());
//		} else {
//			long l = task.getUserId();
//			List<ProjectWorkflow> p = projectWorkflowRepo.findByuser_sapId(l);
//
//			try {
//				for (int i = 0; i < p.size(); i++) {
//					if (p.get(i).getTaskId() == task.getTaskId() && i <= p.size()) {
//						log.debug("M Value {} Task Value {}" , p.get(i).getName() ,task.getTask());
...
//			findModule(task, category, projectAssignation, moduleName, last);
			if(!projectAssignation) {
                long l = category.getUserId();
                List<workflow> m = workflowRepo.findBycategory(l);
